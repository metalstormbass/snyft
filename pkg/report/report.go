...
var categoryTooltips = map[string]string{
	"Publisher Control": "Checks: maintainer count (single = high risk), org vs personal account, verified org membership, maintainer account age (under 6mo flagged), email domain (personal vs organizational), package concentration (50+ packages = high-value target), commit/release signing, org-level MFA enforcement, and npm weekly download volume (1M+/week mitigates single-maintainer risk).",
	...
	"Health":            "Checks: bus factor from commit distribution (how many authors contribute 50% of commits), OSSF Contributors score as fallback, maintainer count as fallback. Also checks review oversight: branch protection with required reviewers or code review rate (75%+). Ecosystems without available data get benefit of doubt.",
	"Governance":        "Checks: repo archived/abandoned status (180+ days inactive = high risk), presence of SECURITY.md or .github/SECURITY.md, OSSF Security-Policy score (5+ out of 10), average issue response time (14 days or less = good), and branch protection with required reviewers or documented release/contributing process as responsiveness fallbacks.",
	"Release Security":  "Checks: CI/CD automated publishing workflow, branch protection on default branch, signed releases, required PR reviews or code review rate, documented release process. Uses OSSF Scorecard as fallback for each. Penalizes: CI/CD workflow risks (unpinned actions, dangerous triggers, script injection, excessive permissions, secrets in logs, missing environment protection) and self-hosted runners.",
	"Package Maturity":  "Checks: package age using repo creation date as primary source (under 6mo = high risk, 6mo-2yr = moderate, 2yr+ = low), staleness since last commit or registry update (1yr+ = high risk, 6-12mo = moderate), and release cadence regularity via coefficient of variation (CV over 2.5 = irregular, only flagged with single maintainer). Final score is the worst of all three.",
}
