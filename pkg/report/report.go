...
// Data sources: web scraping (GitHub/GitLab/Bitbucket pages), bare git clone,
// and package registry metadata (npm/PyPI/Maven). No GitHub API or tokens required.
var categoryTooltips = map[string]string{
	"Publisher Control": "Checks: maintainer count from registry (single = high risk), org vs personal account (scraped), verified org badge (scraped), maintainer account age (under 6mo flagged), email domain (personal vs organizational), package concentration (50+ packages = high-value target), commit signing (git clone), and npm weekly download volume (1M+/week mitigates single-maintainer risk).",
	"Ownership Changes": "Checks: commit author turnover via git clone (recent vs historical committers, 80%+ new = high risk), registry ownership transfer history (npm/PyPI), repo creation vs package publication date mismatch (90+ days = transfer signal), and repository age as fallback.",
	"Release Anomalies": "Checks: dormancy detection via git clone (1yr+ with no commits), dormancy reactivation (1yr+ gap then recent release), relative dormancy (gap 5x+ average cadence), release cadence spikes (release interval under 10% of average), and commit frequency anomalies (year-over-year comparison, e.g. under 5 commits then 20+). Includes safeguards against false positives via data coverage validation and release history cross-validation.",
	"Install Execution": "Checks: presence of install-time script hooks (preinstall, install, postinstall for npm; setup.py for PyPI; pom.xml for Maven), and dangerous pattern analysis in scripts from cloned repos (network requests, filesystem modifications, binary execution, credential exfiltration). Allowlists known build tool patterns: *-config binaries (mysql_config, pg_config, etc.), compilers, and exec(open(...).read()) version-reading idiom. Evidence includes specific line numbers.",
	"Dependency Sprawl": "Checks: transitive dependency count from lock files (package-lock.json, yarn.lock, poetry.lock) with thresholds at 10 and 50 deps. Falls back to direct dependency count from registry metadata (thresholds at 5/15 for npm/PyPI, 12/29 for Maven with scope-aware counting) when no lock file is available.",
	"Provenance":        "Checks: source code availability (public repo URL or source package in artifact), npm provenance attestations, Maven Central GPG signatures (.asc files), and signed release tags (via git clone).",
	"Health":            "Checks: bus factor from commit distribution via git clone (how many authors contribute 50% of commits), maintainer count as fallback. Also checks review oversight: code review rate from scraped PR data (75%+). Ecosystems without available data get benefit of doubt.",
	"Governance":        "Checks: repo archived/abandoned status (180+ days inactive = high risk, scraped from repo page), presence of SECURITY.md or .github/SECURITY.md (git clone), average issue response time (14 days or less = good, scraped), and documented release/contributing process as responsiveness fallbacks.",
	"Release Security":  "Checks: CI/CD automated publishing workflow (from cloned workflow files), signed release tags (git clone), code review rate (scraped PR data), documented release process. Penalizes: CI/CD workflow risks (unpinned actions, dangerous triggers, script injection, excessive permissions, secrets in logs, missing environment protection) and self-hosted runners.",
	"Package Maturity":  "Checks: package age using repo creation date from first commit (under 6mo = high risk, 6mo-2yr = moderate, 2yr+ = low), staleness since last commit or registry update (1yr+ = high risk, 6-12mo = moderate), and release cadence regularity via coefficient of variation (CV over 2.5 = irregular, only flagged with single maintainer). Final score is the worst of all three.",
}
