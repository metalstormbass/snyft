...
3. **Unified Summary** - Per-package synthesis of rule-based scores, deep analysis, and attack patterns with a score adjustment (-2 to +2 points)
4. **Report-Level Summary** - Cross-package synthesis generated after all analysis completes, identifying key threats, cross-cutting patterns, and priority packages across the entire project
...
- Uses 3 focused API calls per package (each with independent 45s timeout) plus 1 report-level summary call
